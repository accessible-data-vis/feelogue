...
namespace Leap.Unity
{
    ...
    /// </summary>
    public class CapsuleHandEdit : HandModelBase
    {
        private long _lockedId = -1; 
        ...
        [MinValue(0)]
        [SerializeField] 
        private float _jointRadius = 0.0004f; //0.008
        ...
        public override void InitHand()
        {
            ...
            {
                if (_backing_material != null) Destroy(_backing_material);
                if (_sphereMat != null) Destroy(_sphereMat);
        ...
        public override void UpdateHand()
        {
            ...
            foreach (var finger in _hand.fingers)
            {
                for (int j = 0; j < 4; j++)
                {
                    ...
                    Vector3 position;
                    if (finger.Type == Finger.FingerType.INDEX)
                    {
                        Index.position = finger.TipPosition;
                        if (Index != null)
                        {
                            SphereCollider collider = Index.GetComponent<SphereCollider>();
                    ...
                    if (finger.Type == Finger.FingerType.THUMB && j == 0)
                    {
                        ...
                        position = _hand.fingers[1].bones[j].PrevJoint;
                        thumb.position = finger.TipPosition;
                    }
                    else
                    {
                    ...
                    if (finger.Type == Finger.FingerType.RING)
                    {
                        fourth.position = finger.TipPosition;
                    }

                    if (finger.Type == Finger.FingerType.PINKY)
                    {
                        fifth.position = finger.TipPosition;
                    }
                    if (finger.Type == Finger.FingerType.MIDDLE)
                    {
                        Middle.position = finger.TipPosition;
                    }
            ...
            {
                Vector3 pinkyMetacarpal = _hand.fingers[4].bones[(int)Bone.BoneType.METACARPAL].PrevJoint;
                Vector3 indexMetacarpal = _hand.fingers[1].bones[(int)Bone.BoneType.METACARPAL].PrevJoint;
